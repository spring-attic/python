...
file system during initialization and load scripts from there.
 ...
 Python app, and push it using the https://github.com/dturanski/java-python-buildpack[Java Python
  Buildpack]. This build pack extends the https://github.com/cloudfoundry/python-buildpack[Python buildpack] to
  support the deployment of a full Python app in the container. The Spring Cloud Stream App will be started and will
  launch a local Python process or run a packaged script using an embedded Jython ScriptEngine, depending on the
  configuration. For example. Suppose you have a directory containing:
